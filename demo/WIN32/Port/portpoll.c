...
xMBPortStartPoolingThread(  )
{
    ...
    if( ( m_hThread =
          CreateThread( NULL, 0, prvdwMBPortPollingThread, NULL, 0,
                        &m_dwThreadId ) ) == NULL )
    {
    ...
    {
        TRACEC
            ( L"CFreeModbusThread::start: waiting for protocol stack to become ready\r\n" );
...
xMBPortStopPoolingThread(  )
{
    ...
    else
    {
        ...
        TRACEC
            ( L"CFreeModbusThread::start: waiting for protocol to shutdown\r\n" );
        eStatus = MB_ENOERR;
    }
    return eStatus;
}
